<!DOCTYPE html>
<html lang="en">
<head>
    <?php echo $__env->make('layouts.header', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
</head>
<body>
    <h2>Data Buku</h2>
    <br>

    <?php if(session('status')): ?>
        <h4><?php echo e(session('status')); ?></h4>
    <?php endif; ?>

    <br>
    <form name="book-save-form" id="book-save-form" action="<?php echo e(url('/books/save-book')); ?>" method="post">
        <?php echo csrf_field(); ?>
        <table>
            <tr>
                <td>ID</td>
                <td>:</td>
                <td><input type="text" name="id" id="id" readonly></td>

            </tr>
            <tr>
                <td>Book Name</td>
                <td>:</td>
                <td><input type="text" name="book_name" id="book-name"></td>

            </tr>
            <tr>
                <td>Author</td>
                <td>:</td>
                
                <td>
                    <select name="author_id" id="author">
                        <option value="">-- Pilih Author --</option>
                        <?php $__currentLoopData = $dataAuthor; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($a['author_id']); ?>"><?php echo e($a['author_name']); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <button type="submit">Save</button>
                    <button type="button" id="button-reset">Reset</button>
                </td>
            </tr>
        </table>
    </form>
    <br>
    <table>
        <tr>
            <th>No.</th>
            <th>ID</th>
            <th>Book Name</th>
            <th>Author</th>
            <th>Published Date</th>
            <th colspan="2">Action</th>
        </tr>
        <?php ($num = 1); ?>
        <?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $b): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <tr class="row-data">
            <td><?php echo e($num++); ?></td>
            <td><?php echo e($b['id']); ?></td>
            <td><?php echo e($b['book_name']); ?></td>
            <td><?php echo e($b['author_name']); ?></td>
            <td><?php echo e($b['published_at']); ?></td>
            <td>
                <button id="button-edit" class="button-edit"
                    data-id="<?php echo e($b['id']); ?>"
                    data-name="<?php echo e($b['book_name']); ?>"
                    data-author="<?php echo e($b['author_id']); ?>">Edit</button>
            </td>
            <td>
                <form action="<?php echo e(url('/books/delete-book?id=').$b['id']); ?>" method="post">
                    <?php echo method_field('DELETE'); ?>
                    <?php echo csrf_field(); ?>
                    <button type="submit">Delete</button>
                </form>
            </td>
        </tr>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </table>

    <script>
        var button = $('.button-edit')

        $(document).ready(function() {
            clearForm()
        });

        button.each(function(index) {
                $(this).on('click', function(){
                    var id = $(this).data('id')
                    var name = $(this).data('name')
                    var author = $(this).data('author')

                    $('#id').val(id)
                    $('#book-name').val(name)
                    $('#author').val(author)
                });
            });

        $('#button-reset').on('click', function () {
            clearForm()
        })

        function clearForm(){
            $('#id').val('')
            $('#book-name').val('')
            $('#author').val('')
        }

    </script>
</body>
</html>
<?php /**PATH C:\xampp\htdocs\Submission 3\perpustakaan-digital-app-master\resources\views/books/index.blade.php ENDPATH**/ ?>